...
ホームの「中継サーバーの設定・接続ヘルプ」、ルーム作成・参加画面および待機画面の「中継サーバーの設定手順」から案内を開けます。接続時に中継設定がない場合は、タイムアウトを待たず、1回の作成・参加操作につき一度だけ自動表示します。APIが取得できない場合は「未作成とは限らない」と区別して案内します。

案内にはMeteredの作成手順とVercelの環境変数・再デプロイ手順を掲載しています。サイト管理者が一度設定するもので、各ゲストがサーバーを契約する必要はありません。「設定を再確認」はAPIから設定を再取得しますが、接続中の通信設定は変更しません。反映後はホストがルームを作り直し、ゲストも参加をやり直してください。案内だけを閉じてもルームの接続処理は続きます。ルーム作成・参加ダイアログを閉じるとキャンセルします。

UI検証：`node tests/relay_setup_ui.cjs`（ブラウザ実行環境の指定は上記と同じ）。

## PC同士は成功し、スマホを含む接続が失敗する場合

2026-09-09の公開API確認ではHTTP 200、`relayConfigured:false`、`reason:not-configured`、STUNのみでした。この状態では回線のNATやファイアウォールで直接接続が制限されると中継できません。端末の種類だけでは原因を断定できないため、同一Wi-Fiと別回線を分け、両端末の診断も確認します。

接続ヘルプの「この端末の中継接続を検査」は、新しい設定を取得して中継限定のWebRTC接続候補を収集します。12秒以内にrelay候補が取得できれば、その端末からTURNへの到達を確認できます。相手とのDataChannel接続やレース通信の成功は別途確認が必要です。案内を閉じると検査用接続は破棄され、参加中のルームには影響しません。

中継候補を取得できない場合は認証期限・利用上限・サービス状態を確認し、サービスが実際に提供するTCP/TLS経路も設定してください。URLを書き換えて未提供のポートを追加しても解決しません。設定反映後、ホストがルームを作り直し、PC→スマホ、スマホ→PC、スマホ→スマホで双方の画面を開いたまま確認します。

ICEのfailed状態はタイムアウトを待たず再試行します。disconnectedは一時的な回線変化でも発生するため即失敗扱いにはしません。検証：`node tests/relay_probe.mjs`、`node tests/p2p_recovery.mjs`。
