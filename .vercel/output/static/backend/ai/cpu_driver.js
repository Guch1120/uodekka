...
import { activateSkill } from '../skills/unique_skills.js';
...
export class CPUDriver {
  static stepAI(aiKart, dt, courseTrack, courseConfig, knowledgeBase, gameState) {
    ...
    // ミッション形式スキルアップシステムによる最高速向上をAI目標速度にも反映
    targetSpeed = targetSpeed * (physics.missionTopSpeedMult || 1);
    ...
    // 5. 加速／ブレーキ／ドリフト決定
    let accel = 0.95 * (personality.speedScale || 1.0) * (physics.missionAccelMult || 1);
    ...
    if (useItemTrigger && physics.holdingItem) {
      ...
      gameState?._trackerByPhysicsId?.get(physics.id)?.onItemConsumed();
    ...
    // 7. 固有スキル自動発動（コース別CPU学習Lv.5以上のみ、走行中かつ再使用可能なら発動）
    if (aiKart.skillState && aiKart.skillState.cooldown <= 0 && aiKart.skillState.activeTimer <= 0
      && !physics.isRespawning && !physics.isFinished) {
      const vehicle = Vehicles.types[personality.vehicleKey];
      if (vehicle?.skill) {
        activateSkill(physics, aiKart.skillState, vehicle.skill, aiKart.missionTracker);
      }
    }
